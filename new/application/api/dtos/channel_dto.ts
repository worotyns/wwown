import {
  ...
  import { ChannelData, ChannelDataParams } from "../../../domain/stats/channel/channel_data.ts";
  
   type ChannelSummaryLabel =
  | "reactions" // How many reactions are you receiving?
  | "reactions" // How many reactions are you giving?
  | "threadCount" // How many threads are you participating in?
  | "threadCount" // How many threads are you authoring?
  ...
  export class ChannelViewDto {
    ...
    static getSummary(extendedStats: ChannelData, params: ChannelDataParams) {
      const summary: SerializableMap<ChannelSummaryLabel, [SummaryDescription, Total, Total]> =
        new SerializableMap();
      ...
      summary.set('msgsCount', ['Total messages in channel', msgsTotalInRange, msgsTotalAllTime]);
      ...
      summary.set('channelsCount', ['Unique users participating in conversations', channelsCountInRange , channelsCountAllTime]);
      ...
      const threadMessagesAllTime = Array.from(allTimeThreads.values()).reduce((a, b) => a + b.total, 0);
      const threadMessagesInRange = days.reduce((a, b) => a + Array.from(b.threads.values()).reduce((a, b) => a + b.total, 0), 0);
      ...
      const threadMaxMessagesAllTime = Array.from(allTimeThreads.values()).reduce((a, b) => Math.max(a, b.total), 0);
      const threadMaxMessagesInRange = days.reduce((a, b) => Math.max(a, Array.from(b.threads.values()).reduce((a, b) => Math.max(a, b.total), 0)), 0);
  
      summary.set('threadMaxMessages', ['Maximum messages per thread', threadMaxMessagesInRange, threadMaxMessagesAllTime]);
  
      const threadAvgMinutesAllTime = Array.from(allTimeThreads.values()).reduce((a, b) => a + Math.ceil((b.firstTs.getTime() - b.lastTs.getTime()) / 3_600_000), 0) / threadCountAllTime;
      const threadAvgMinutesInRange = days.reduce((a, b) => a + Array.from(b.threads.values()).reduce((a, b) => a + Math.ceil((b.firstTs.getTime() - b.lastTs.getTime()) / 3_600_000), 0), 0) / threadCountInRange;
      summary.set('threadAvgMinutes', ['Average thread duration', threadAvgMinutesInRange, threadAvgMinutesAllTime]);
  
      const threadMaxMinutesAllTime = Array.from(allTimeThreads.values()).reduce((a, b) => Math.max(a, Math.ceil((b.firstTs.getTime() - b.lastTs.getTime()) / 3_600_000)), 0);
      const threadMaxMinutesInRange = days.reduce((a, b) => Math.max(a, Array.from(b.threads.values()).reduce((a, b) => Math.max(a, Math.ceil((b.firstTs.getTime() - b.lastTs.getTime()) / 3_600_000)), 0)), 0);  
      summary.set('threadMaxMinutes', ['Maximum thread duration', threadMaxMinutesInRange, threadMaxMinutesAllTime]);
  
      const reactionsAllTime = Array.from(allTimeReactions.values()).reduce((a, b) => a + b.total, 0);
      const reactionsInRange = days.reduce((a, b) => a + Array.from(b.reactions.values()).reduce((a, b) => a + b.total, 0), 0);
  
      summary.set('reactions', ['Total reactions', reactionsInRange, reactionsAllTime]);
    ...
    static getReactionsAllTime(extendedStats: ChannelData) {
      ...
      for (
        ...
        emoji = ChannelViewDto.normalizeEmoji(emoji);
